...
class StarlightCrushGame {
    ...
    createBoard() {
        ...
        for (let row = 0; row < this.boardSize; row++) {
            ...
            for (let col = 0; col < this.boardSize; col++) {
                ...
                // Touch ve Mouse event'lerini ekle
    ...
    handleTouchEnd(event) {
        ...
        if (this.isDragging && this.dragEndPos) {
            const deltaX = this.dragEndPos.x - this.dragStartPos.x;
            const deltaY = this.dragEndPos.y - this.dragStartPos.y;
            const direction = this.getDragDirection(deltaX, deltaY);
            
            if (direction) {
                this.performSwap(direction);
            }
        }
